...
# Self-contained installer for the Ong-Or FastAPI systemd service.
# Run from the repo root on the board:
#   sudo bash ./install_systemd_service.sh --user ongor --group ongor \
#       --workdir /opt/ongorAI --python /usr/bin/python3

SERVICE_NAME="ongor-fastapi"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
UNIT_SRC="${SCRIPT_DIR}/deploy/ongor-fastapi.service"
ENV_EXAMPLE="${SCRIPT_DIR}/deploy/ongor-fastapi.env.example"
...
if [[ ! -f "$UNIT_SRC" ]]; then
	echo "Missing ${UNIT_SRC}. Make sure the full repo was copied to the board." >&2
	exit 1
fi

if [[ ! -d "$WORKDIR" ]]; then
	echo "Workdir not found: $WORKDIR" >&2
	echo "Current board path appears different. Check the project location first." >&2
	exit 1
fi
